...
Vor einer Fehlerauswertung kann nicht auf den Wert zugegriffen werden (siehe \autoref{rust:result}).

\todo{..}
Ein Sperrversuch kann einem Fehler beantwortet werden, wenn ein vorheriger Thread die Sperre nicht ordnungsgemäß, sondern mit einer \rustcinline{panic!}, beendet hat.
In diesem Fall wird ein \rustcinline{PoisonError} zurückgegeben, der einen direkten Zugriff auf den Wert verhindert und darauf hinweist, dass der geschützte Wert vermutlich in keinem konsistenten Zustand mehr ist.
Daraufhin kann versucht werden, wieder einen konsistenten Zustand zu erlangen.

\todo{..}
Ähnlich wie in C++ mit \ccinline{local_guard}, wird auch in Rust ein exklusive Zugriff durch das erlangen einer Sperre ermöglicht.
...
Es wird nicht wie traditionell einen Codebereich, sondern einen Wert geschützt:
...
\todo{\enquote{To isolate unsafe code as much as possible, it’s a good idea to enclose unsafe code within a safe abstraction and provide a safe API} \url{https://doc.rust-lang.org/book/second-edition/ch19-01-unsafe-rust.html}}
