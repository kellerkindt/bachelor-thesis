...
Newsletter.prototype = {
		saveSubscription : function(){
			...
			var handlerFunc = function(t) {
				...
				if(response.code == 1){
					...
					// use display block instead of show() which can generate display inline
					$('#'+nsltrObj.errorElementId).css("display","block");
					$('#'+nsltrObj.successElementId).hide();
				}
				else {
					$('#'+nsltrObj.successElementId).html(response.message);
					// use display block instead of show() which can generate display inline
					$('#'+nsltrObj.successElementId).css("display","block");
		...
		validateEmail : function(i18nInvalidEmail){
			if($('#'+this.newsletterElementId).val().match(/^[a-zA-Z0-9_+&*-]+(?:\.[a-zA-Z0-9_+&*-]+)*@(?:[a-zA-Z0-9-]+\.)+[a-zA-Z]{2,9}$/gi)==null){
				$('#'+this.errorElementId).html(i18nInvalidEmail);
				$('#'+this.errorElementId).css("color","red");
				$('#'+this.errorElementId).css("display","block");
...
//		    $('#'+stateDropdownId).append($('<option></option>'));
	...
	validateEmail : function(fieldId, invalidMessage, emailExistMessage) {
		...
		if(!emailStr.match(/^[a-zA-Z0-9_+&*-]+(?:\.[a-zA-Z0-9_+&*-]+)*@(?:[a-zA-Z0-9-]+\.)+[a-zA-Z]{2,9}$/gi)){
	...
	identicaNonEmptyPasswords : function(p1Id, p2Id, noMatchMessage) {
		...
		if($('#' + mainCheckboxId).is(':checked') == true) {
			$('#'+mainCheckboxId+'_en').attr("disabled", false);
			$('#'+mainCheckboxId+'_zh').attr("disabled", false);
			$('#'+mainCheckboxId+'_ja').attr("disabled", false);
			$('#'+mainCheckboxId+'_pt').attr("disabled", false);
			$('#'+mainCheckboxId+'_fr').attr("disabled", false);
		}
		else {
            $('#'+mainCheckboxId+'_en').prop('checked', false);
			$('#'+mainCheckboxId+'_en').attr("disabled", true);
            $('#'+mainCheckboxId+'_zh').prop('checked', false);
            $('#'+mainCheckboxId+'_zh').attr("disabled", true);
            $('#'+mainCheckboxId+'_ja').prop('checked', false);
            $('#'+mainCheckboxId+'_ja').attr("disabled", true);
            $('#'+mainCheckboxId+'_pt').prop('checked', false);
            $('#'+mainCheckboxId+'_pt').attr("disabled", true);
            $('#'+mainCheckboxId+'_fr').prop('checked', false);
            $('#'+mainCheckboxId+'_fr').attr("disabled", true);
		}
	},

    toggleIndustryInterests: function(cbIndustryEmail){
        if($('#' + cbIndustryEmail).is(':checked') == true) {
            $('#userprofile_industry_interests').attr("disabled", false);
        }else{
            $('#userprofile_industry_interests').prop('checked', false);
            $('#userprofile_industry_interests').attr("disabled", true);
        }
    },
	...
	validateSocialRegisterForm : function(emailPreValidated) {		
		...
		if(emailPreValidated) {
			// validate email that came from social idp, just for safety!!!!, for twitter email is already validated at this point
			this.validateEmail("userprofile_email", "Invalid email", "Email already exists");
		...
		if((typeof loggedIn != "undefined") && loggedIn){
			if(InfoQConstants.isUserBlocked==true || InfoQConstants.isUserOutdated==true)
			{
				...
				if(typeof userCurrentCompany !== "undefined"){
					...
					if(userSubscribedArchitectNewsletterEN){
                        $('#architectNewsletterSection').hide();
					}
					if(userSubscribedIndustry){
						$('#indemailSection').hide();
                        $('#industryInterestsSection').hide();
					}
	...
	forceUpdateExecuteCallback: function(){
		...
		if(typeof forceUpdateCallback !=='undefined'){
			if(typeof forceUpdateCallback ==="function"){
                ...
                while(forceUpdateCallback.slice(-1) == "#") {
                    forceUpdateCallback = forceUpdateCallback.slice(0,-1);
                }
	...
	initRegistrationGDPR : function() {
		$(document).ready( function() {
			if(($('#userprofile_country').val() == '-1000') || ($('#userprofile_country').val() == '-1')){
				$('.registration-newsletterlist').hide();
			}

			//initialize newsletter subscriptions based on the country detected
			if($('#userprofile_country').length > 0) {
				UserActions_Profile.optNewslettersBasedOnCountryGDPR($('#userprofile_country').val());
			}

			//for social registration, select newsletter subscriptions based on country from idp
			if($('#initialCountryValue').length > 0) {
				UserActions_Profile.optNewslettersBasedOnCountryGDPR($('#initialCountryValue').val());
			}


			$('#userprofile_country').change(function(){
				if(($(this).val() == '-1000')||($(this).val() == '-1')){
					$('.registration-newsletterlist').hide();
					return true;
				}
				$('.registration-newsletterlist').show();
				UserActions_Profile.optNewslettersBasedOnCountryGDPR($(this).val());

			});
		});
	},

	optNewslettersBasedOnCountryGDPR : function(countryId) {
		if((countryId == '14')||(countryId == '21')||(countryId == '23')||(countryId == '97')||(countryId == '54')||(countryId == '55')||(countryId == '58')||(countryId == '63')||(countryId == '70')||(countryId == '75')||(countryId == '56')||(countryId == '88')||(countryId == '99')||(countryId == '101')||(countryId == '108')||(countryId == '132')||(countryId == '130')||(countryId == '131')||(countryId == '148')||(countryId == '161')||(countryId == '174')||(countryId == '179')||(countryId == '184')||(countryId == '196')||(countryId == '194')||(countryId == '67')||(countryId == '191')||(countryId == '77')){
			$('#userprofile_newsletter').prop('checked', false);
			$('#userprofile_newsletter_en').prop('checked', false);
            $('#userprofile_newsletter_en').attr("disabled", true);
			$('#userprofile_newsletter_zh').prop('checked', false);
            $('#userprofile_newsletter_zh').attr("disabled", true);
			$('#userprofile_newsletter_pt').prop('checked', false);
            $('#userprofile_newsletter_pt').attr("disabled", true);
			$('#userprofile_newsletter_ja').prop('checked', false);
            $('#userprofile_newsletter_ja').attr("disabled", true);
			$('#userprofile_newsletter_fr').prop('checked', false);
            $('#userprofile_newsletter_fr').attr("disabled", true);
			$('#userprofile_architect_newsletter').prop('checked', false);
			$('#userprofile_industry_email').prop('checked', false);
            $('#userprofile_industry_interests').prop('checked', false);
            $('#userprofile_industry_interests').attr("disabled", true);
			//for VCR "Registration info" for clients - uncheck by default if country is GDPR
			$('#showOther').prop('checked', false);
			try {showHideOtherValues($('#showOther'));}catch(err) {}
		}
		else {
			$('#userprofile_newsletter').prop('checked', true);
			$('#userprofile_newsletter_' + InfoQConstants.language).prop('checked', true);
            $('#userprofile_newsletter_en').attr("disabled", false);
            $('#userprofile_newsletter_zh').attr("disabled", false);
            $('#userprofile_newsletter_pt').attr("disabled", false);
            $('#userprofile_newsletter_ja').attr("disabled", false);
            $('#userprofile_newsletter_fr').attr("disabled", false);
			$('#userprofile_architect_newsletter').prop('checked', true);
			$('#userprofile_industry_email').prop('checked', true);
            $('#userprofile_industry_interests').prop('checked', true);
            $('#userprofile_industry_interests').attr("disabled", false);
			//for VCR "Registration info" for clients - check by default if country is non-GDPR
			$('#showOther').prop('checked', true);
			try {showHideOtherValues($('#showOther'));}catch(err) {}
		}
	}
};
...
UserActions_RestrictedPresentations = {
		...
		shareTokenValid : function(tokenValue) {
			if(tokenValue){
				re = /^[a-zA-Z0-9_+&*-]+(?:\.[a-zA-Z0-9_+&*-]+)*@(?:[a-zA-Z0-9-]+\.)+[a-zA-Z]{2,9}$/gi;
		...
		validateEmails : function(csvEmails) {
			if(!trim(csvEmails)==""){
				...
				for(var i=0;i<emails.length;i++){
					if(!emails[i].match(/^[a-zA-Z0-9_+&*-]+(?:\.[a-zA-Z0-9_+&*-]+)*@(?:[a-zA-Z0-9-]+\.)+[a-zA-Z]{2,9}$/gi)){
						this.hasErrors=true;
					}
				}
			}
...
var Tracker = {
	...
	doTrackVcrImpressions : function() {
		...
		setTimeout(function () {
			$(".f_dfp_ad:visible").each(function(){
				...
				$.when(appearedInView($this, 1)).then(
					function(status) {
						if(Tracker.isImpressionJsonValid(viewableImpressionJson)) {
							Tracker.metricsArray.push(viewableImpressionJson);
							Tracker.checkIfMetricsCanBePosted();
	...
	encodeNotificationLinks: function (container) {
		...
		$(container).find("a").each(function() {
            ...
            if($(this).hasClass("f_notif_type_upcmtonmcnt")) {
                campaignType="user_posted_comment";
            }
    ...
    doTrackPSBoxEvents : function(action, label) {
		if(!Tracker.trackPSBoxEvents) {
			return;
		}
        _gaq.push(['_trackEvent', 'P&Sbox', action, label]);
    }
...
var UserActions_Personalization = {
    followTopic: function (isFollowing, id, btnId, primaryTopicLabel) {
        UserActions_Personalization.requestFollow(isFollowing, id, btnId);
        if(!isFollowing) {
            _gaq.push(['_trackEvent', 'Follow topic', 'follow', primaryTopicLabel]);
        } else {
            _gaq.push(['_trackEvent', 'Follow topic', 'unfollow', primaryTopicLabel]);
        }
    },
    ...
    followSmall: function (followVarName, id, target, f, group, hoverTopicLabel) {
        UserActions_Personalization.requestFollowSmall(followVarName, id, '#'+target+f+id, target, group);
        if(target == "Topic" && hoverTopicLabel !== undefined && hoverTopicLabel !== null && hoverTopicLabel !== "") {
            if (window[followVarName]) {
                _gaq.push(['_trackEvent', 'Follow topic', 'follow', hoverTopicLabel]);
            } else {
                _gaq.push(['_trackEvent', 'Follow topic', 'unfollow', hoverTopicLabel]);
            }
        }
    },
    ...
    errorMessage: function () {
        console.log("ERROR: Please contact support! Thank you.");
    },
    ...
    prepareGroupFollowData: function (hoverTopicLabel) {
        theMap = {
            "Topic": ptCatIdList,
            "User": authorIdList,
        ...
        if(theMap.Topic.length > 0) {
            UserActions_Personalization.setupGroupFollowButtons(theMap.Topic, "Topic", hoverTopicLabel);
        }
    ...
    setupGroupFollowButtons: function (idList, target, hoverTopicLabel) {
        if (idList.length > 0) {
            $.each(idList, function (idx, value) {
                ...
                $('a[id="' + tvalue + '"]').each(function (i, v) {
                    if (!loggedIn) {
                        ...
                        $(this).click(function () {
                            UserActions_Personalization.followSmall('isFollowed' + tvalue, value, target, '', 'All',hoverTopicLabel);
                            return false;
                        });
                    }
                });
...
var PersonalizationSocialAd = {
    checkDisplayDialog : function() {
        /* display dialog only if cookie is not set */
        if(CookieManager.readCookie('PSAdialog') !== 'closed') {
            $("#psa-note").css("display", "block");
            /* configure link for ad */
            if(loggedIn && loggedIn == true) {
                $("#psa_note_lnk1").attr('href', 'https://www.infoq.com'+ InfoQConstants.countryCode +'/notifications/settings');
                $("#psa_note_lnk1").click(function() {
                    Tracker.doTrackPSBoxEvents("editors picks", "desktop");
                });
            } else {
                $("#psa_note_lnk1").addClass('authModal--trigger');
                $("#psa_note_lnk1").click(function() {
                    Tracker.doTrackPSBoxEvents("login", "desktop");
                });
            }
            /* set cookie when closing dialog */
            $("#psa-close").click(function() {
                CookieManager.createCookie('PSAdialog', 'closed', 15552000000);
                Tracker.doTrackPSBoxEvents("close", "desktop");
                $("#psa-note").hide();
            })
        }
    }
...
var TopBarPromoWidget = {
    init : function() {
        cookieName = 'topbarSurvey';
        if (CookieManager.readCookie(cookieName) !== 'hide') {
            $(document).ready(function () {
                $('.top-bar-promo').slideDown();
                $('.top-bar-close').click(function () {
                    $('.top-bar-promo').slideUp();
                    CookieManager.createDayCookie(cookieName, 'hide', 3650);
                });
            });
        }
    }
