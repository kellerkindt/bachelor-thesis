...
var P = { // Video Player
    ...
    adjustSpeed : function(speed, element) {
        ...
        $('.presentation_speed_link').removeClass('active');
    ...
    seekPlayerTime : function(time) {
        var timeInSeconds = hmsToSeconds(time);
        if(!P.canPlayHTML5()) {
            ...
            try {
                $('#video').get(0).currentTime = timeInSeconds;
            } catch (exc) { console.log("Cannot skip to HTML5 video position"); }
        }
    }
};
...
$(function () {
    if(!P.canPlayHTML5() && InfoQConstants.language !== 'zh') {
        initFlashPlayerPresentation();
    }
	// preload all images
	try{
