    ...
    > AlgorithmShim<I, E, S, FE, FI>
{
    pub unsafe fn new(
        ...
        environment_frame_publisher: FE,
        init_message_publisher: FI,
    ) -> Result<Box<AlgorithmShim<I, E, S, FE, FI>>, ()> {
        let mut algorithm = Box::new(AlgorithmShim {
            shim: {
    ...
    unsafe extern "C" fn publish_environment_frame(
        ...
        let myself: &mut AlgorithmShim<I, E, S, FE, FI> = ::std::mem::transmute(instance);
        let frame: &E = ::std::mem::transmute(frame as *mut E);
        myself.environment_frame_publisher.publish(frame);
    }
    ...
    unsafe extern "C" fn publish_init_message(
        ...
        myself.init_message_publisher.publish(frame);
    }
