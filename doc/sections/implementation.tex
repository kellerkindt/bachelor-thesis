...
Das \enquote{Message Queuing Pattern} soll für die interne Kommunikation zwischen \textit{Algorithm} und \textit{Client} bzw. zwischen \textit{Algorithm} und \textit{CountListener} und \textit{EnvironmentListener} verwendet werden
...
In Zeile 5 wird ein neuer Kanal erzeugt und eine Sende- und \todo{Empfängerinstanz} zurück gegeben.
Zwischen Zeile 7 und 9 wird eine neuer Thread erzeugt und der auszuführenden Closure (siehe \autoref{rust:closures}) die Eigentümerschaft über den Empfänger übertragen (Schlüsselwort \rustcinline{move}).
Der neuer Thread wartet nach Start auf den Empfang einer neuen Nachricht und gibt diese anschließend auf der Konsole aus.
In Zeile 11 sendet der Haupt-Thread über den Kanal dem anderen Thread einen String\footnote{Um genau zu sein, ist der Wert vom Datentyp \rustcinline{&'static str} \todo{.}} zu und wartet in Zeile 12 bis dieser beendet wurde.
