...
\todo{Closures Lambdas: großes Thema, nur anschneiden? ... es gibt etwas wie Lambdas, nennt sich Closures... besonders weil move / Eigentümerschaft übernehmen, Fn/FnMut/FnOnce + Notation nicht erwähnten?}
...
Die einfachste Schleife ist \rustcinline{loop \{ \} }: der Rumpf der Schleife wird ohne Bedingung wiederholt.
...
Eine inkrementelle Laufvariable, für zum Beispiel die Indexierung eines Arrays, wird durch das Iterieren über einen Zahlenstrahl dargestellt.
...
Für Unit-Tests müssen Module und Funktionen mit entsprechenden Anmerkung versehen sein, Integrationstests müssen im Verzeichnis \rustcinline{tests/} gespeichert sein \cite{rust:book:tests}.
...
Durch diese Markierung wird der beinhaltete Code nur für das Ausführen der Tests compiliert.
...
Von einer weiteren Vertiefung wird hier abgesehen.
Bei der Implementierung des Servers wird versucht, die Benennung und Formatierung konform umzusetzen.
...
Falls der Compiler keine ordnungsgemäße Nutzung feststellen kann, wie zum Beispiel eine Referenz, die länger als die eigentliche Wert leben möchte, wird die Kompilation verweigert.
...
Das Beispiel in \autoref{rust:ownership:scope} zeigt verschieden Möglichkeiten und Beschränkungen beim verleihen und übertragen von Werten.
...
Eine falsche Nutzung der API wird durch einen Compilerfehler bemängelt, anstatt eines zur Laufzeit unerwarteten Verhaltens.

\rustcinline{String::from_utf8(vec: Vec<u8>)} nimmt zum Beispiel einen \rustcinline{Vec<u8>} entgegen und konsumiert diesen.
Der Aufrufer kann danach nicht mehr auf den diesen Zugreifen, da die Eigentümerschaft an die Funktion \rustcinline{from_utf8} übertragen wurde.
...
Ein typisches Problem, die Modifikation einer Kollektion während einer Iteration, wird auch durch das Eigentümerprinzip verhindert.
\todo{Einen exklusiven Zugriff auf eine Kollektion zu erhalten -- um einen Wert einzufügen, zu entfernen oder zu ändern, wenn über diese iteriert wird, ist nicht möglich, da der Iterator die Kollektion bereits lesend geliehen hat.}
Anstatt das eine \ccinline{ConcurrentModificationException} zur Laufzeit geworfen wird, unterbindet dies der Rustcompiler durch eine Fehlermeldung \cite[116]{rust:orly_programming}.

\section{\todo{zu viel + unnötig?} Rust als funktionale Programmiersprache ??s}
