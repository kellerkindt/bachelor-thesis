...
Das Eigentümerprinzip (siehe \autoref{rust:ownership}) erlaubt eine sichere \todo{und vergleichsweise einfache} Nebenläufigkeit,
...
\todo{crates.io -> docs.rs}

\todo{rust-lang.org -> doc.rust-lang.org}

\todo{https://github.com/rust-lang}

\todo{https://play.rust-lang.org/}
...
Zum anderen wird der Aufbau als Crate empfohlen \todo{cite}, bei dem automatisch Abhängigkeiten aufgelöst aber auch Metainformationen bezüglich des Autors, der Version und Abhängigkeiten hinterlegt sind.
...
Mit dem erzeuge einer Crate (\rustcinline{cargo --bin meinProg} bzw. \rustcinline{cargo --lib meineBib}) wird auch gleichzeitig \gls{git} für das Verzeichnis initialisiert.
...
In Rust haben Ausrufezeichen und Fragezeichen besondere Bedeutungen, weswegen die Verwendung in Zeile 2 trotzdem ist.
...
Während dies in Programmiersprachen wie Java und C\# automatisch durch \todo{vtables} umgesetzt wird (\todo{virtuelle methoden, vergleich c++, speicher, indirekt}), muss in Rust dies der Entwickler entscheiden.
\todo{dabei dabei dabei dabei dabei dabei} Dabei gibt es mehrere Vorgehensweisen:
\begin{itemize}
	...
	\item Datentyp, der das geforderte Merkmal implementiert, auf den Heap verschieben und die \todo{Eigentümerschaft} über den Heapbereich übertragen (\todo{resultiert in einfacher Pointer-übergabe}): \rustcinline{fn foo(bar: Box<Bar>)} (auch \enquote{Trait-Object} genannt)
...
Der zu reservierende Speicher für die Variable kann nicht bestimmt werden.

Eine spezialisierte Funktion verhält sich ähnlich wie eine \todo{Templateklasse} in C++.
Der Compiler erzeugt für jede Spezialisierung eine Kopie der Funktion und setzt den entsprechenden Typ ein.
Dies ermöglicht auch Optimierungen der Funktion für den eingesetzten Typ \todo{cite?}, vergrößert aber das Compilat.
...
\begin{itemize}
	...
	\rustcinline{!Sized} verhindert dagegen, dass ein Wert zu anderen Threads übertragen werden darf.
	Somit können ansonsten textuelle Beschränkungen, wie für der OpenGL-Kontext, durch den Compiler überprüft und erzwungen werden. C\todo{Beispiel OpenGL?}
	
	\item \monospaceinline{Sync}: Markiert einen Datentype als Thread sicher, d.h. mehrere Threads dürfen gleichzeitig lesend darauf zugreifen, während \rustcinline{!Sync} dies verhindert. \todo{automatisch? manuell?} Verlangt, dass alle beinhalteten Datentypen auch \monospaceinline{Sync} sind.
	...
	\item \rustcinline{PartialOrd}: Verlangt die Implementation einer Funktion um mit Instanzen des gleichen Typs sortieren zu können. Erlaubt aber auch, dass Werte zueinander nicht sortierbar sind.
...
\begin{itemize}
	...
	\todo{graph stack(pointer) + heap(vec+elements) + stack(slice)}
	\begin{figure}[H]
		\centering
		\begin{tikzpicture}
...
Die Ausnahme macht dabei die Nutzung von \rustcinline{unsafe}, für zum Beispiel FFI (siehe \autoref{rust:ffi}).
...
Rust dagegen lässt keinen zugriff auf Variablen zu, die nicht zuvor initialisiert wurden \cite[126]{rust:orly_programming}.
...
\rustcinline{Option<_>} ist eine Aufzählung, die entweder \rustcinline{None} ist, oder \rustcinline{Some(_)} mit einem Wert.
...
Der \rustcinline{Option<_>} Datentyp kann zum Beispiel tatsächlich als Pointer dargestellt werden, der bei \ccinline{NULL} \rustcinline{None} ist und ansonsten \rustcinline{Some(_)} \cite[100]{rust:orly_programming}.
Somit wird eine Überprüfung erzwungen, ohne dabei Laufzeitkosten erzeugt zu haben.

\todo{nothing special?}
Ein weiteres Beispiel ist der atomare Referenzzähler  von \rustcinline{Arc<_>}. Der Zähler ist im Heap direkt vor dem beinhalteten Wert und nicht in einem extra Speicherbereich \todo{cite}.
...
\begin{figure}[H]
	...
	\draw[black, dotted] (8, 2.5) -- (12, 2.5);
	\node[text width=3cm, align=center] at (10, 2) {Actual Value};
	\draw[black, dotted] (8, 1.5) -- (12, 1.5);
	\node[text width=3cm, align=center] at (10, 1) {\ldots};
	\draw[black, dotted] (8, 0.5) -- (12, 0.5);
	
	
	
	\draw[decoration={brace,mirror,raise=5pt},decorate] (1, 3.5) -- node[left=6pt] {Arc} (1, 2.5);
	%\draw[decoration={brace,mirror,raise=5pt},decorate] (1, 2.45) -- node[left=6pt] {Slice} (1, 0.5);
	...
	\end{tikzpicture}
	\caption{Speicherlayout Arc \todo{cite}}
\end{figure}
