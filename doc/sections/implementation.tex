...
\todo{OPTIONALE ANFORDERUNGEN 33*0}
...
\todo{.}
Das \enquote{Message Queuing Pattern} (dt. \todo{}) gilt als stabiles und einfach umzusetzendes Entwurfsmuster, um zwischen asynchron arbeitenden Komponenten oder Klassen Nachrichten auszutauschen \cite[207]{douglass2003real}.
Es gilt allerdings als sehr schwergewichtig, da im klassischen Sinne nur Werte \enquote{by-value} ausgetauscht werden sollen (in einem \enquote{by-reference} System ist das Resourcenmanagement deutlich komplizierter und Speicherlecks oder \enquote{double-free}s könnten auftreten). 
...
In Zeile 5 wird ein neuer Kanal erzeugt und eine Sende- und Empfängerinstanz zurück gegeben.
Zwischen Zeile 7 und 9 wird eine neuer Thread erzeugt und der auszuführenden Closure \todo{ref} die Eigentümerschaft über den Empfänger übertragen (Schlüsselwort \rustcinline{move}).
Der neuer Thread wartet nach Start auf den Empfang einer neuen Nachricht und gibt diese anschließend auf der Konsole aus.
In Zeile 11 sendet der Haupt-Thread über den Kanal dem anderen Thread eine Textnachricht zu und wartet in Zeile 12 bis dieser beendet wurde.
...
Weiterer C-Code soll den C++ Code über eine C-Datenstruktur kapseln und über C-Funktionen zugänglich machen \todo{Facade!}.
