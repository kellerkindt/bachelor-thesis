...
mod test {
    ...
    #[test]
    fn deferred_spawn_is_executed() {

        let mut runtime = tokio::runtime::Runtime::new().expect("Failed to create runtime");
        let mutex = Arc::new((Mutex::new(0_u32), Condvar::new()));
        {
            let mutex_clone = mutex.clone();
            let mut mutex_lock = mutex.0.lock().expect("Failed to retrieve lock");

            let once = stream::once(Ok(()));
            let mut deferred = DeferredSpawn::new(
                once.for_each(move |_| {
                    let mut lock = mutex_clone.0.lock().expect("Failed to retrieve lock in for_each");
                    *lock += 1;
                    mutex_clone.1.notify_all();
                    Ok(())
                }).into_future()
            );


            assert_eq!(0, *mutex_lock);
            runtime.spawn(deferred);

            mutex_lock = mutex.1.wait(mutex_lock).expect("Failed to wait");
            assert_eq!(1, *mutex_lock);
        }

        runtime.shutdown_now().wait().expect("Failed to shutdown");
    }

    #[test]
    fn deferred_spawn_spawns_only_one_time() {
        let mut runtime = tokio::runtime::Runtime::new().expect("Failed to create runtime");
        let mut deferred = DeferredSpawn::new(empty_future());

        runtime.spawn(stream::once(Ok(())).for_each(move |_| {
            assert_eq!(Ok(Async::Ready(())), deferred.poll());
            assert_eq!(Err(()), deferred.poll());
            assert_eq!(Err(()), deferred.poll());
            Ok(())
        }));
    }

    fn empty_future() -> impl Future<Item=(),Error=()> {
        stream::once(Ok(())).for_each(|_| Ok(()))
    }
}