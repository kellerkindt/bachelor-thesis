...
Im ersten Testszenario führt die Implementation in Rust eine in Rust geschriebenen \enquote{Dummy}-Algorithmus aus (siehe Anforderung \reqNumberForLabel{impl:algorithmus} in \autoref{req:impl:algorithmus}).
...
\todo{::} Falls eine Serverimplementation mit dieser Stresssituation umgehen kann, erfüllt sie auch die Anforderung \reqNumberForLabel{exec:time} im Produktivbetrieb.
...
Aus dem Diagramm ist zu erkennen, das sowohl die C++ Implementation, als auch die Rust Implementationen, trotz erhöhtem Stress, die Anforderung \reqNumberForLabel{exec:time} (siehe \autoref{req:exec:time}) erfüllen.
...
Wie in \cite[160]{douglass2003real} beschrieben, wird durch die parallele Verarbeitung von mehreren Elementen in einem Kanal (in diesem Fall \todo{AlgorithmAdapter} zu \todo{C++ Algorithmus} zu \todo{Fahrzeugen}) die Verarbeitung einer hohen Datenmenge ermöglicht.
