...
Programmiersprachen scheinen auf der Suche nach dem nicht existierenden \enquote{Nirvana} zu sein, das sowohl sichere als auch brauchbare Programmierung verspricht \cite[ab ca Minute 58:20]{rust:infoq:null}.
...
\begin{itemize}
	...
	\item \textbf{beta}: Alle sechs Wochen werden Features aus der Nightly in eine neue Beta übernommen.
	Anschließend werden nur noch Fehler aus dieser Version getilgt.
...
%Zum jetzigen Zeitpunkt ~März~ sind über 14.000 Crates öffentlich erreichbar und nutzbar.
...
Zum einen gibt es den klassischen Aufbau, in dem lediglich der Programmcode liegt und der Compiler direkt aufgerufen und parametrisiert wird.
...
\begin{wrapfigure}{L}[-1em]{.5\textwidth}
	\rustcinclude
		...
		{Verzeichnisstruktur des\newline Quelltext-Verzeichnisses}
...
In Rust haben Ausrufezeichen und Fragezeichen besondere Bedeutungen, weswegen die Verwendung in Zeile 2 so richtig ist.

Die Bedeutung des Fragezeichens dient zum schnelleren Auswerten von \rustcinline{Result<_, _>}-Werten und wird in \autoref{rust:result} genauer erklärt.
...
Dort haben die primitiven Datentypen (\ccinline{short}, \ccinline{int}, \ccinline{long}, ..) keine definierte Größe, sondern sind abhängig vom eingesetzten Compiler und der Zielplattform \cite[187]{deitel2013c}. Erst ab C99 wurden zusätzliche, aber optionale, ganzzahlige Datentypen mit fester Größe definiert \cite[141]{goll2014c}.
...
Eine Schreibweise in Binär (\rustcinline{0b0000_1000_u8}), in Hexadezimal (\rustcinline{0xFF_08_u16}) oder in Oktal (\rustcinline{0o64_u8}) ist ebenso möglich. 
Konstante Zeichen und Zeichenketten können automatisch durch ein vorangestelltes \textit{b} in Bytes gewandelt werden: \rustcinline{b'b'} entspricht \rustcinline{0x62_u8} und \rustcinline{b"abc"} entspricht \rustcinline{&[0x61_u8, 0x62_u8, 0x63_u8]}.
...
Variablen werden durch das \rustcinline{let} Schlüsselwort gebunden. Das heißt, der Variablen wird die Eigentümerschaft für den Wert zugewiesen.
Ausnahmen können Datentypen mit dem Merkmal \rustcinline{Copy} bilden, da diese eine implizite Kopie erlauben (siehe \autoref{rust:generics}).
...
Selbst wenn kein expliziter Rückgabetyp angegeben ist, wird formal \rustcinline{()} zurückgegeben; \rustcinline{()} entspricht etwa \ccinline{void} aus bekannten Programmiersprachen.
...
Dies liegt daran, dass in Rust vieles ein Ausdruck ist und somit einen Rückgabewert liefert \cite{rust:book:statements}.
Auch ein if-else ist ein Ausdruck und kann einen Rückgabewert haben.
Ein bedingter Operator (?:) ist somit unnötig, da stattdessen ein if-else verwendet werden kann: \rustcinline{let a = if b \{ c \} else \{ d \};}.
...
In Rust wird dieser Typ \enquote{Trait} (dt. Merkmal) genannt.
...
\begin{itemize}
	...
	Somit können ansonsten rein textuell beschriebene Beschränkungen, wie zum Beispiel für den OpenGL-Kontext, durch den Compiler überprüft und erzwungen werden.
	
	\item \monospaceinline{Sync}: Markiert einen Datentyp als zwischen Threads synchronisierbar, d. h. mehrere Threads dürfen gleichzeitig lesend darauf zugreifen.
	\rustcinline{!Sync} verbietet dies hingegen.
	Wird automatisch für alle Datentypen implementiert, bei denen auch alle beinhalteten Datentypen von Typ \rustcinline{Sync} sind.
	...
	\item \monospaceinline{Debug} und \monospaceinline{Display}: Erzwingt die Implementation von Funktionen, um einen Datentyp als Text darzustellen. Entweder mit möglichst vielen Zusatzinformationen (\monospaceinline{Debug}) oder auf das Wesentliche reduziert (\monospaceinline{Display}).
...
\begin{itemize}
	\item \rustcinline{Fn<Args>} mit \rustcinline{fn call(&self, arg: Args) -> Self::Output}: Ein mehrfacher Aufruf ist möglich, ohne dass Nebeneffekte auftreten, da die Eigenreferenz unveränderlich ist.
...
\rustcinclude
	{rust:match:iflet}
	{Vereinfachter \rustcinline{if let} Ausdruck}
...
Das Beispiel in \autoref{rust:loops:while} gibt eine Sekunde lang wiederholend \monospaceinline{Zeit noch nicht um} auf der Konsole aus.
...
Die Makros \rustcinline{assert!(a)}, \rustcinline{assert_eq!(a, b)} und \rustcinline{assert_ne!(a, b)} prüfen Ergebnisse und lösen \rustcinline{panic!} aus (siehe \autoref{rust:panic}), falls Ergebnisse nicht den erwarteten Werten entsprechen.
...
Rust kennt \ccinline{NULL}(-Pointer) nicht und erlaubt auch keinen Zugriff auf nicht initialisierte Variablen (siehe \autoref{rust:no_unitialized_usage}), bietet aber einen \rustcinline{Option<_>}-Datentyp als Ersatz an.
...
In Rust wird weder eine Ausnahme geworfen, noch ein Rückgabewert zurückgegeben, der ohne Prüfung verwendet werden kann:
...
Dazu muss die Funktion, die den Operator verwendet, aber auch ein \rustcinline{Result} mit einem kompatiblen Fehlertyp zurückgeben.
...
Die Speichersicherheit umfasst aber auch den Zugriff auf Puffer. So ist es nicht möglich einen Pufferüberlauf zu provozieren, da die Größe von einem Array und einer Slice immer bekannt ist.
...
Trotz der vielen verwendeten Abstraktionen möchte Rust dadurch möglichst keine weiteren Laufzeitkosten erzeugen.
...
\begin{itemize}
	...
	\item Falls der Aufbau wie in \autoref{rust:ffi:example:enumerate:repr} unbedeutend ist, aber ausdrücklich auf einen externen Datentyp hingewiesen werden soll, kann dieser in einem \rustcinline{extern \{ \} } Block bekannt gemacht werden: \rustcinline{extern \{ type PositionOffset; \}} \cite{rust:github:extern_type}.
...
Mehrere Versuche die Layoutberechnung in C++ zu parallelisieren sind aufgrund schwer auffindbarer Fehlern abgebrochen worden \cite{rust:example:firefox}.
Eine Parallelisierung im aktuellen Projekt \enquote{Quantum} in Rust ist dagegen mit ersten Erfolgen gekrönt \cite{rust:example:firefox_heise}
