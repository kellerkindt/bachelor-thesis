...
Konstante Zeichen und Zeichenketten können auch automatisch durch ein vorangestelltes \textit{b} in Bytes gewandelt werden (\rustcinline{b'b'} entspricht \rustcinline{0x62_u8} und \rustcinline{b"abc"} entsricht \rustcinline{&[0x61_u8, 0x62_u8, 0x63_u8]}).
...
Die Größe einer Slice wird dabei mit der Referenz auf die Startposition gespeichert und oft auch \enquote{fat pointer} genannt (siehe auch \autoref{rust:memory_layout:vec_slice}).
Ein zusätzliche, manuell gepflegte Variable für die Größe eines Arrays bzw Pointer auf ein Array, wie in C üblich, ist somit unnötig.
Die Notation ähnelt dabei die eines Arrays, aber ohne Größenspezifikation: \rustcinline{&[<Datentyp>]}.
Eine Slice ist zudem immer eine Referenz.
...
Variablen werden durch das \rustcinline{let} Schlüsselwort \enquote{gebindet}, das heißt, der Variable wird die Eigentümerschaft über den Wert zugewiesen.
...
Da Rust ein statisches Typensystem mit lokaler Typinferenz ist \todo{cite? https://www.youtube.com/watch?v=-Tj8Q12DaEQ}, muss der Datentyp der Variable nicht notiert werden, sondern dieser wird automatisch erkannt.
...
\todo{... just rewrite it}
Auf Zusammengesetze und einfache Datentypen können Referenzen erstellt werden.
Es wird zwischen einer exklusiven, veränderlichen Referenz und vielen, unveränderlichen Referenzen unterschieden (siehe \autoref{rust:ownership}).
Die Notation unveränderlicher Referenzen ist \rustcinline{&<Datentyp>}, von veränderlichen Referenzen ist dies \rustcinline{&mut <Datentyp>}.
Für Merkmale wird zukünftig vermutlich zusätzlich das Schlüsselwort \rustcinline{dyn} in den Syntax aufgenommen (siehe \autoref{rust:generics}).
Referenzen auf Referenzen sind möglich.
Referenzen sind immer gültig, es gibt keine \ccinline{NULL}-Referenz (siehe \autoref{rust:no_null}).
...
Falls doch nötig, dann mit \rustcinline{*}.
...
\todo{...}
In Zeile 3 von \autoref{rust:match:large} wird \rustcinline{value} aufgelöst und auf der Konsole wird anschließend \monospaceinline{Wert ist: text} ausgegeben.
In dem Beispiel ist \rustcinline{value} aus Zeile 2 und 3 \rustcinline{Some("text")}.
Sowohl Zeile 4 als auch Zeile 5 prüfen auf das übereinstimmende Feld \rustcinline{Some}, aber nur der Codepfad in Zeile 5 wird ausgeführt.
%Dekonstruktion von Werten mittels Pattern Matching
Dies liegt an der Prüfung auf den beinhalteten Wert, der in Zeile 4 mit \rustcinline{\"test\"} übereinstimmen müsste.
Da eine Übereinstimmung nicht vorliegt, trifft als nächstes Zeile 5 zu, in der nur das Feld \rustcinline{Some} übereinstimmen muss, während der beinhaltete Wert beliebig sein darf.
Die Variable \rustcinline{value} bindet bei einer Übereinstimmung den Wert, um ihn für den Programmcode ansprechbar zu machen.
Falls dies nicht nötig ist, kann auch das Wildcard \rustcinline{_} verwendet werden.

Das \rustcinline{match} Statement von Rust verlangt, dass eine Musterabgleichung immer zu einem Ergebnis führt, dementsprechend müssen all Felder eine Aufzählung aufgeführt sein.
Hiermit wird verhindert, dass, nachdem eine Aufzählung um ein Feld erweitert wurde, eine Musterabgleichung nicht um das neue Element ergänzt wurde.
In Situationen, in denen nur wenige Fälle von Bedeutung sind, können alle anderen Fälle durch das Wildcard mittels \rustcinline{_ => {}} adressiert werden.

Wenn sogar nur ein konkreter Fall von Bedeutung ist, kann dies in der verkürzten Schreibweise \rustcinline{if let} notiert werden:
...
Da dies zur Compilezeit geschieht, ist eine Überprüfung zur Laufzeit nicht nötig, weshalb diese Philosophie keinen negativen Einfluss auf die Laufzeit hat.
...
Da eine \rustcinline{match} oder ein \rustcinline{if let} für jeden Funktionsaufruf der einen Fehler zurückgeben könnte, sehr umständlich und bereits für kleine Beispiele wie \autoref{rust:result:rust_good_fopen} unübersichtlich wird, kann dies durch den Operator \rustcinline{?} abgekürzt werden.
Dazu muss die Funktion, die den Operator verwendet aber auch ein \rustcinline{Result} im einem kompatiblen Fehlertyp zurückgeben, wie in \autoref{rust:result:shorthand} zu sehen:
